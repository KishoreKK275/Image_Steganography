...
void handle_encode(struct mg_connection *c, struct mg_http_message *hm) {
    ...
    if (img_len < 2 || img[0] != 'B' || img[1] != 'M') {
        ...
        // Primary: Use magick convert (for ImageMagick v7+)
        int conv_result = system("magick convert temp_input temp.bmp");
        if (conv_result != 0) {
            LOG("Magick convert failed with exit code: %d, trying legacy convert", conv_result);
            // Fallback: Try plain convert (for older versions, but may fail on Windows)
            conv_result = system("convert temp_input temp.bmp");
            if (conv_result != 0) {
                LOG("Image conversion failed with both magick convert and convert");
